...
export default function BlockDAGVisualization() {
  ...
  const render = useCallback(() => {
    ...
    if (settings.showEdges) {
      console.log('Drawing edges - showEdges:', settings.showEdges, 'blocks:', allBlocksRef.current.length);
      let edgeCount = 0;
      allBlocksRef.current.forEach(block => {
        if (block.parentIds && block.parentIds.length > 0) {
          const toPos = blockPositions.get(block.id);
          if (!toPos) {
            console.log(`Missing position for block ${block.id}`);
            return;
          }
          ...
          block.parentIds.forEach(parentId => {
            ...
            if (!fromPos) {
              console.log(`Missing position for parent ${parentId} of block ${block.id}`);
              return;
            }
            edgeCount++;
